...
float plasma(vec2 coord, float t) {
    float v = 0.0;
    v += sin(coord.x * 10.0 + t);
    v += sin((coord.y * 10.0 + t) * 0.5);
    v += sin((coord.x * 10.0 + coord.y * 10.0 + t) * 0.5);
    
    vec2 c = coord * 10.0 - vec2(5.0);
    v += sin(sqrt(c.x * c.x + c.y * c.y + 1.0) + t);
    
    c = coord * 10.0 - vec2(5.0 + sin(t * 0.5) * 3.0, 5.0 + cos(t * 0.7) * 3.0);
...
void main() {
    ...
    vec3 base = mix(customColor1, customColor2, 0.5);

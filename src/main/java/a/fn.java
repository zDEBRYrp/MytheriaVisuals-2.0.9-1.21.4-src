...
public final class fn implements IMinecraft {
   ...
   public static Vec2f worldToScreen(Vec3d worldCoords) {
      if (a != null && b != null) {
         ...
         Vec3d var2 = worldCoords.subtract(var1.getPos());
         Vector4f var3 = new Vector4f((float)var2.x, (float)var2.y, (float)var2.z, 1.0F);
         var3.mul(a).mul(b);
         if (var3.w <= 0.0F) {
            return null;
         }

         Vector4f var4 = var3.div(var3.w);
         float var5 = (var4.x + 1.0F) / 2.0F * mc.getWindow().getScaledWidth();
         float var6 = (1.0F - var4.y) / 2.0F * mc.getWindow().getScaledHeight();
         return new Vec2f(var5, var6);
      } else {
         return null;
      }
   }

...
@ModuleInfo(name = "Mine Helper", category = ax.PLAYER, desc = "Помощник в шахте")
public class bp extends aJ {
   private final ch a = new ch(this, "Сохранять кирку", "Не дает сломать блок, если предмет достиг определенной прочности").enable();
   ...
   private final EventListener<P> e = event -> {
      if (mc.player != null) {
         ItemStack var2 = mc.player.getMainHandStack();
         if (this.a(var2)) {
            double var3 = this.b(var2);
            if (this.a.isEnabled() && !(var3 >= this.percent.getCurrentValue())) {
               event.cancel();
            }
         }
      }
   ...
   private final EventListener<W> f = event -> {
      if (mc.player != null && this.c) {
         Mytheria.getInstance().getRotationHandler().rotate(new fD(mc.player.getYaw(), 90.0F), fC.SILENT, 180.0F, 180.0F, 180.0F, fG.USE_ITEM);
      }
   };

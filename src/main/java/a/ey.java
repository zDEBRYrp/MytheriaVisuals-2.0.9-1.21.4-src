...
public class ey<T extends ex> {
   protected final List<T> slots;

   public ey(List<T> slots) {
      this.slots = slots;
   }
   ...
   public T findEmptySlot() {
      return this.slots.stream().filter(ex::isEmpty).findFirst().orElse(null);
   }
   ...
   public ey<ex> and(ey<? extends ex> other) {
      ArrayList var2 = new ArrayList(this.slots.size() + other.slots.size());
      var2.addAll(this.slots);
      var2.addAll(other.slots);
      return new ey<>(var2);
   }

   public ey<ex> and(ex slot) {
      ArrayList var2 = new ArrayList<>(this.slots);
      var2.add(slot);
      return new ey<>(var2);
   }

...
public class ft extends fo {
   private final GlProgram a;
   private float b = 0.5F;

   public ft() {
      super(VertexFormats.POSITION_COLOR);
      this.a = fj.rectangleProgram;
   }

   public ft smoothness(float s) {
      this.b = s;
      return this;
   }
   ...
   public void add(Matrix4f matrix, float x, float y, float width, float height, float radiusTL, float radiusBL, float radiusTR, float radiusBR, int rgba) {
      ...
      this.getBuilder().vertex(matrix, var13, var14, 0.0F).color(rgba);
      this.getBuilder().vertex(matrix, var13, var14 + var16, 0.0F).color(rgba);
      this.getBuilder().vertex(matrix, var13 + var15, var14 + var16, 0.0F).color(rgba);
      this.getBuilder().vertex(matrix, var13 + var15, var14, 0.0F).color(rgba);
   }
}

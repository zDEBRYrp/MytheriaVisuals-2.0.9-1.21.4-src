...
@ModuleInfo(name = "Death Cords", category = ax.PLAYER, desc = "Отправляет координаты смерти в чат")
public class aU extends aJ {
   private boolean a;
   private final ch b = new ch(this, "Ставить метку");
   private final EventListener<W> c = event -> {
      if (!(mc.currentScreen instanceof DeathScreen) || mc.player == null) {
         ...
         eg.info(Text.of("Координаты смерти: " + var2 + " " + var3 + " " + var4));
         if (this.b.isEnabled()) {
            cv var5 = Mytheria.getInstance().getWayPointsManager();
            if (var5.contains("Death")) {
               var5.del("Death");
            }

            var5.add("Death", var2, var3, var4);
         }
